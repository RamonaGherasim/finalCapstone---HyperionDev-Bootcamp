...
def read_shoes_data():
    """
    This function reads the shoe inventory from external file.
    If file is available, the function will open it in read mode, read the
    lines and close.
    If the file has information in it, each line is split and
    the information is assigned to the variables needed to then create an
    object of Shoe class. Each object is appended to the shoe list.
    If the file has no information in it, user will receive an error message
...
def capture_shoes():
    """
    This function takes user input for the attributes needed to create a Shoe
    object. A while loop is used to check that the user input for cost and
    quantity is correct (a positive integer)
    If all the data needed for the attributes is introduced correctly, a Shoe
    object is created and appended to the shoe list. This information is then
    appended in the text file (*please see notes at the bottom) and a success
    message is printed. The loop will break.
    If the user introduces data that is not valid for cost and quantity, error
    messages will be displayed and the loop will continue asking the user for
    input.
    """
...
def view_all():
    """
    This function will take each object in the shoe list and call the string
    method for it, then append this information in shoe data list.
    The information in this shoe data list is then displayed in a table format
    using the Python tabulate module.
    """
...
def re_stock():
    """
    This function will display the item with the lowest current stock (in table
    format) and gets user input on the quantity to be added to the stock.
    An infinite while loop is used to check for the validity of the stock input
    If the input is valid (it is a positive integer, this will be added to the
    current stock amount and a success message is printed. The loop will break.
    If the input is not a valid one, error messages will be displayed and user
    is asked for input again.
    When the stock is modified, the new information will be updated in the text
    file.
    """
    quantities_list = []
    for shoe in shoe_list:
...
"""
DOCUMENTATION:
https://www.geeksforgeeks.org/how-to-make-a-table-in-python/#:~:text=The%20tabulate%20%28%29%20method%20is%20a%20method%20present,installed%20using%20the%20below%20command%20pip%20install%20tabulate
I have used this website to learn about the tabulate Python module. I learnt 
how to install the module using pip in the console, the parameter it needs and
how to use it to display data in the desired table format.

NOTES:
I have added the writing (appending) in file part for the capture_shoes() 
function as I believe it makes sense to be able to use the data that user 
inputs further, if they would like to add stock or see it in the stock list, 
etc. 
"""